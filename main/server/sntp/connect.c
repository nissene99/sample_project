...
esp_err_t example_connect(void)
{
	...
	ESP_LOGI( prefix, "Waiting for IP(s)");
	...
	for (int i = 0; i < esp_netif_get_nr_of_ifs(); ++i) {
		...
		if (is_our_netif( prefix, netif)) {
			ESP_LOGI( prefix, "Connected to %s", esp_netif_get_desc( netif));
			ESP_ERROR_CHECK(esp_netif_get_ip_info(netif, &ip));

			ESP_LOGI( prefix, "- IPv4 address: " IPSTR, IP2STR( &ip.ip));
#ifdef CONFIG_EXAMPLE_CONNECT_IPV6
...
esp_netif_t *get_example_netif_from_desc(const char *desc)
{
	esp_netif_t *netif = NULL;
	char *expected_desc;
	asprintf( &expected_desc, "%s: %s", prefix, desc);
